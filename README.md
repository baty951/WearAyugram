...
- **Сообщения**:
  ...
  - реакции — видны на пузырях (своя подсвечена), долгое нажатие открывает пикер для отправки/снятия
- **Ayugram-фичи**:
...
- Реплаи и пересылка — не реализованы; кастомные эмодзи-реакции не отображаются
