...
- **Сообщения**:
  ...
  - пересылка — из меню действий с выбором чата-получателя (с поиском)
  - опросы — голосование с часов, включая множественный выбор и викторины; результаты обновляются живьём
  - геопозиция, место, контакт, кубик, звонки, анимированные эмодзи — текстовые представления
  - сервисные сообщения («вступил», «закреплено», «тема создана»…) — серой строкой по центру
- **Ayugram-фичи**:
...
- **Мультиаккаунт** — до 3 аккаунтов с раздельными TDLib-базами, переключение и выход из настроек (через перезапуск процесса)
...
- Кастомные эмодзи-реакции не отображаются
